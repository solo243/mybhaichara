...
const homeRandomCache = new Map();

export async function getRandomHomeVideos(limit = 8) {
  const cacheKey = `home-random:${limit}`;
  if (homeRandomCache.has(cacheKey)) {
    return homeRandomCache.get(cacheKey);
  }
  ...
  try {
    ...
    const result = rawVideos.map((video) => ({
      ...video,
      _id: video._id.toString(),
    }));

    homeRandomCache.set(cacheKey, result);
    return result;
  } catch (error) {
