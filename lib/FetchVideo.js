...
// export const getRecommendedVideos = async (currentId, limit = 12) => {
  ...
  const fetchCachedRecommendations = unstable_cache(
    async (cId, lmt) => {
      try {
        ...
        console.error("Failed to fetch recommendations:", error);
        return []; // Fail gracefully so the page still loads
      }
    },
    [`recommended-videos-${currentId}-${limit}-${process.env.BUILD_ID || "1"}`],
    {
      revalidate: 180,
      tags: ["recommendations"],
    },
